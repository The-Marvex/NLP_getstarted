...
{	
    ...
    tokenize_sentence("sanjeev human pankaj neeraj shobhit rddhima anushtha rahul himanshu kjkljh");
}
...
void tokenize_sentence(string sentence)
{
	...
	cout<<index_vector[0]<<index_vector[1]<<index_vector[2]<<index_vector[3]<<"vgvd vector is cute"<<endl;

	for(int i = 0;i<index_vector.size();)
	{
		cout<<index_vector[i]<<"  "<<index_vector[i+1] <<endl;
		i = i+2;
	}
	...
	while(m<=index_vector.size()/2)
	{
		...
		//cout<<" gets size of vector"<<index_vector[m+1]<<" "<<index_vector[m]<<" "<<temp_size<<endl;
		char arr[temp_size];
		int start = p;
